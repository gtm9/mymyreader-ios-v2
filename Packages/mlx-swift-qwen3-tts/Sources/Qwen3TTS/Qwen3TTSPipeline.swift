...
/// Cache for precomputed voice embeddings to bypass heavy encoder stages.
public struct TTSPromptCache: Sendable {
    /// The 1024-dimensional speaker embedding
    public let speakerEmbedding: [Float]
    
    public init(speakerEmbedding: [Float]) {
...
public struct Qwen3TTSPipelineConfiguration: Sendable {
    ...
    /// Whether to load the optional audio encoder (used for ICL / in-context voice cloning).
    ///
    /// Loading this can significantly increase memory usage because it loads additional
    /// weights from the speech tokenizer safetensors file.
    public var loadAudioEncoder: Bool
    ...
    public init(
        ...
        self.loadAudioEncoder = loadAudioEncoder
...
public final class Qwen3TTSPipeline: @unchecked Sendable {
    ...
    public init(modelPath: URL, configuration: Qwen3TTSPipelineConfiguration = .default) throws {
        ...
        let result: (Qwen3Talker, Qwen3Tokenizer, AudioDecoder, SpeakerEncoder?, Qwen3TTSAudioEncoder?, Qwen3TTSConfig) = try Device.withDefaultDevice(device) {
            ...
            if configuration.loadAudioEncoder {
                do {
    ...
    /// Build a prompt cache from reference audio to bypass encoding on subsequent runs.
    public func buildPromptCache(from audioSamples: [Float]) -> TTSPromptCache? {
        guard let embedding = extractSpeakerEmbedding(audioSamples: audioSamples) else {
            return nil
        }
        return TTSPromptCache(speakerEmbedding: embedding)
    }
